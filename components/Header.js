...
function Header() {
  ...
  // connection du composant avec le reducer pour récupérer les infos de connexion 
  ...
  // maj de la date du jour à chaque initialisation du composant
  useEffect(() => {
    setDate(new Date());
  }, []);
  ...
  // vider le reducer pour que les bookmarks disparaissent à la déconnection 
  const handleLogout = () => {
    dispatch(logout());
  ...
  if (user.token) {
    ...
    // Si la modale est visibile il peut la quitter en cliquant sur la croix
    ...
    if (isModalVisible) {
      userSection =
        ...
        // Si la modale n'est pas visible il peut l'afficher en cliquant sur l'icone profil
